...
const AdminSidebar=()=> {
  ...
  return (
   <div className='h-[calc(100vh-64px)] md:flex flex-col items-center pt-8 w-60 md:w-60
    border-r border-gray-300/20 text-sm'>
        ...
        <div className='w-full'>
            {adminNavlinks.map((link, index)=>(
                ...
                ${isActive && 'bg-red-500/15 text-red- group'}`}>
                    {({ isActive })=>(
                        <>
                        <link.icon className='w-5 h-5'/>
                        <p className='max-md:hidden'>{link.name}</p>
                         <samp className={`w-1.5 h-10 rounded-l right-0 absolute ${isActive && 'bg-red-500'}`} />
                        </>
                    )}
                </NavLink>
            ))}
